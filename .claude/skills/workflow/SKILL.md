---
name: workflow
description: How work moves through bb-kit — the beads ledger, the verify gate, the review pass, and the token budget that keeps them affordable on one account. Use when starting a work session, deciding where a note or a discovery belongs, planning a multi-step change, handing off at the end of a session, or wondering which skill or script to reach for.
---
...
**Name the bead in the commit message** — `Closes bbk-abc`, or just the id in the
