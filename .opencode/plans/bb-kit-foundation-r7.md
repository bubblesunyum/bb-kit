# bb-kit — Foundation Plan r7

**Date:** 2026-08-30 · **Replaces:** r6 and everything before it. **This is the version to build from.** · **Repo:** `/Users/bubbles/dev/bb-kit` (empty) · **Old kit for reference:** `../bubble-kit` (React Native)

---

## Start here

You are the agent building this. Before writing any code:

1. Read §2 in full. Those seven rules decide most of the judgement calls you will hit, and they are not negotiable.
2. Read §9 for the order of work. Do not reorder it — the sequence exists because two earlier drafts got it wrong.
3. §4 and §5 contain exact values. Transcribe them; do not re-derive them.
4. Where the plan says to check something against current documentation — the shadcn registry template, the registry item format, Storybook addon names, whether `@radix-ui/react-slot` is browser-only — actually check, and **write what you found in the pull request**. Those are the places most likely to have moved.
5. Where you must guess, say so in the pull request rather than guessing quietly.

The owner of this kit will never read your source. Their only way to judge the work is looking at it in Storybook, so a thing that is not visible there is a thing that does not exist.
...
**Changed since r6:** the clashing palette now has real hex values, generated and verified rather than described (§5.2). Base UI is out of the install list and Radix's `Slot` is in (§9). Added the base border rule that dropped-in shadcn components need (§4.2). Dropped `Badge`'s redundant `filled` variant.

**Changed in r6 (kept for history):** restored the Tailwind rule that makes dropped-in shadcn components work (§4.2). Fixed a color pair that actually failed (§5.1). Gave the clashing palette a dark mode (§4.5). Turned §3.5 from a variant list into a full prop table. Made the mode default robust rather than order-dependent (§4.1). Restored the card-against-page check (§4.6). Added tests that enforce the two rules the design rests on (§9). Split `Theme` in two. Renamed `outlined` to `outline`. Dropped Base UI.

**Changed in r5 (kept for history):** the clashing palette was impossible as written and is respecified (§4.5). The mode setting now has a default, so "no mode" follows the reader's system rather than silently meaning light (§4.1). Tailwind's `dark:` prefix is banned inside the kit, because it reintroduced the nesting bug §4.1 exists to fix (§4.2). Variant names, values and defaults are written down (§3.5). Shadows were using a mechanism that only accepts colors (§5.6). `InfiniteList` is deferred. Registry setup is now an actual step and gets tested (§9). About twenty smaller corrections throughout.
...
`Badge` has no `variant` at all. Its only two appearances are unselected (outline) and selected (filled with primary), and those are *states*, not variants — a `filled` variant would be a second way to say `selected`, which is what this table exists to prevent.
...
**Sizes.** `Button`, `Badge` and `SearchField` take `size`: `sm` · `md` · `lg`, default `md`. Heights 32 / 40 / 48px, all clearing the 24px minimum in §5.6.
...
| `Badge` | `size` · `selected` · `onSelectedChange(next: boolean)` · `disabled` |
...
- **Alias shadcn's variable names to ours.** Anyone running `npx shadcn add dialog` gets a component reading `--background`, `--foreground`, `--muted-foreground`, `--ring`, `--popover` and so on. Our roles have different names, so it would render unstyled. Inside `@theme inline` the names are prefixed, so the aliases look like this — write them in exactly this shape, because `--background: var(--page)` would generate no utility and report nothing:
  ...
  **Aliases are not enough on their own.** Tailwind v4 changed the bare `border` utility to use the current text colour. shadcn components write plain `border` and rely on a base rule pointing it at the border token. Without that rule a dropped-in dialog gets near-black hairlines in light mode and near-white in dark — it renders, it just looks wrong, and nothing reports it. Add:
...
- **It inverts in both modes.** Light mode has a *dark* page. Dark mode has a *light* page.

Both halves are needed. Light mode inverted catches anything assuming "light mode means a pale background" and anything assuming the primary is the dark one. Dark mode inverted catches the mirror — anything assuming "dark mode means a dark background." And §7.2's four-up view shows both palettes across both modes, so leaving either half undefined would leave a cell to guesswork.

**The actual hex values are in §5.2**, generated and checked the same way the default palette was. An earlier draft described this palette in words and left the values to whoever built it — which would have meant inventing thirty-four colours that clear fifty-four thresholds, failing the test, and being tempted to lower a threshold to go green. Thresholds never move. Colours do.
...
**The focus ring is checked against page and card, not against the primary fill.** That is because of how it is drawn: an outline sitting *outside* the control, on the surface behind it (§5.6). It never lands on the button's own fill, so contrast against the fill is not the question. If you ever switch to an inset ring, this row has to change.
...
**So the hex values in the table above are the source of truth.** They live in the palette TypeScript file, hand-fixed. `scripts/build-palette.ts` does not generate them; it *measures* them, printing every §4.6 pair with its number. **A working reference implementation ships next to this plan** as `verify-forest.mjs` and `verify-clash.mjs` — run them with `node`, confirm both print ALL PASS, and port them into the repo's test. Regenerating colours from the ramp would produce different values than the table. Regenerating the contrast numbers from the script is not only fine but required, since r5 claimed numbers it had not measured and one of them was wrong.

### 5.2 The clashing palette

Not a product. A test, per §4.5. Warm hue 35 throughout (danger at 27), high colorfulness, and **inverted in both modes** — light mode has a dark page, dark mode has a light page.

| Role | Light (dark page) | Dark (light page) |
|---|---|---|
| Page | `#371C15` | `#F5E4DF` |
| Card | `#4F2E26` | `#FEFBFA` |
| Text | `#FDF2F0` | `#29140F` |
...
### 5.3 OKLCH to write, OKLab to blend
...
### 5.6 Corners, shadows, motion, target size
...
**Not** Base UI. An earlier draft installed it and never used it. It was there for `asChild`, which `CardLink` replaced (§3.3), and nothing in v0 needs a headless UI library — there is no dialog, select, popover or menu.

`asChild` now comes from `@radix-ui/react-slot`, which is what shadcn itself uses. **Check whether the installed version ships a `'use client'` marker.** If it does, importing it drags `Card`, `ListItem` and `CardLink` into the browser and §3.8's server-renderable list becomes quietly false — the exact silent failure §3.8 warns about. In that case either move those three to the browser-only list, or write the forty-line `Slot` by hand. Say which you did in the pull request.
...
Then both palettes — the hex values are given in §5.1 and §5.2, so this is transcription — the check function from §4.6, a test running it over every palette and mode, and the `/tokens` page. The test should pass on the first run; if it does not, the transcription is wrong, not the colours.
...
1. **The theme system (§4).** The core design — one palette attribute plus an inherited `color-scheme`, read by `light-dark()` — has been checked across several reviews and holds. Least tested now: the split shadow colours in §5.6, the shadcn interop in §4.2 (aliases, the `dark:` variant and the base border rule), and whether `@radix-ui/react-slot` forces components into the browser (§9 Step 0). All three have a verification step attached; do them before building on top.
