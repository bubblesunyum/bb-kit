...
def git_state():
    ...
    if upstream:
        ...
        for entry in [e for e in raw.split("\x1e") if e.strip()]:
            ...
            for bead in set(re.findall(r"\bbbk-[a-z0-9]+(?:\.\d+)?\b", entry)):
...
def main():
    ...
    if command == "shot":
        target = args[1] if len(args) > 1 else "/tmp/bbk-dashboard.png"
    ...
    if command == "up":
        ...
        if not note:
            print(f"! nothing came up on {port} — see " + "/tmp/bbk-dash.log")
        else:
